...
public class CabinetMedicalController {
    ...
    @GetMapping("/cabinets/rendezvous")
    public ResponseEntity<CabinetMedical> getCabinetRendezvous(Authentication authentication){
        String username = authentication.getName();
        if(username == null){
            throw new RuntimeException("Veuillez saisir un login valide");
        }
        CabinetMedical cabinetMedical = cabinetMedicalService.getCabinetMedicalByadminUsername(username);

        return ResponseEntity.ok(cabinetMedical);
    }
}